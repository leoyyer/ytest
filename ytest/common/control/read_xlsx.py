...
class ReadXlsData:
    ...
    def get_filename(self, filename):
        ...
        if os.path.exists(filename):
            ...
            NOW_EXCEL_DETAIL = {"file_name": filename, "case_name": case_name, "type": dir_list[-1], "project": dir_list[-2]}
            return NOW_EXCEL_DETAIL
        else:
            # 如果excel文件不存在，则抛出FileError异常
            raise exc.FileError(f'{filename} 不存在,请检查')
        ...
        try:
            ...
            for n in range(1, table.nrows):
                ...
                try:
                    ...
                    raise exc.CaseGenerateError(f'公共断言格式不正确,请检查: {assert_str}') # 如果无法转换为字典，报异常
    ...
    def get_sql_detail(self):
        ...
        try:
            ...
            result_list = []  # 存储所有合法的断言字典
            for n in range(1, table.nrows):
                assert_str = resolve_vars(table.cell_value(n, 0),self.global_variable)
                try:
                    assert_dict = json.loads(assert_str)  # 尝试将sql字符串转换为字典
                    result_list.append({f"{n}":assert_dict})
                except json.JSONDecodeError:
                    raise exc.CaseGenerateError(f'sql格式不正确,请检查: {assert_str}') # 如果无法转换为字典，报异常
            self.exl['base'].update({"sql_list":result_list})
        except Exception as e:
            # 如果出现异常，则引发CaseGenerateError异常
            raise exc.CaseGenerateError(f'sql初始化失败,请检查: {e}')
        ...
        try:
            ...
            for n in range(1, table.nrows):
                ...
                if var_type in type_converter:
                    ...
                    GLOBAL_VARIABLE[f"{table.cell_value(n, 0)}"] = json.loads(table.cell_value(n, 1))
    ...
    def get_case_data(self) -> list:
        ...
        for n in range(1, table.nrows):
            ...
            case_detail = {}
            update_dict = {
                0: ("case_id", resolve_vars(table.cell_value(n, 0),self.global_variable)),
                1: ("title", resolve_vars(table.cell_value(n, 1),self.global_variable)),
                2: ("is_run", resolve_vars(table.cell_value(n, 2),self.global_variable)),
                3: ("model", resolve_vars(table.cell_value(n, 3),self.global_variable)),
                4: ("level", resolve_vars(table.cell_value(n, 4),self.global_variable)),
