        ...
        try:
            ...
            for n in range(1, table.nrows):
                ...
                try:
                    ...
                    raise exc.CaseGenerateError(f"sql格式不正确,请检查: {assert_str}")
                    # 如果无法转换为字典，报异常
        ...
        * 仅做数据读取,不做数据判断(is_run = 否,也正常读取,留给pytest判断是否要跳过)
        ...
        for n in range(1, table.nrows):
            ...
            update_dict = {
                0: (
                    "case_id",
                    resolve_vars(table.cell_value(n, 0), self.global_variable),
                ),
                1: (
                    "title",
                    resolve_vars(table.cell_value(n, 1), self.global_variable),
                ),
                2: (
                    "is_run",
                    resolve_vars(table.cell_value(n, 2), self.global_variable),
                ),
                3: (
                    "model",
                    resolve_vars(table.cell_value(n, 3), self.global_variable),
                ),
                4: (
                    "level",
                    resolve_vars(table.cell_value(n, 4), self.global_variable),
                ),
                ...
                9: (
                    "headers",
                    str_to_dict(
                        resolve_vars(table.cell_value(n, 9), self.global_variable)
                    ),
                ),
                10: (
                    "cookies",
                    str_to_dict(
                        resolve_vars(table.cell_value(n, 10), self.global_variable)
                    ),
                ),
                11: (
                    "param",
                    str_to_dict(
                        resolve_vars(table.cell_value(n, 11), self.global_variable)
                    ),
                ),
                12: (
                    "body",
                    str_to_dict(
                        resolve_vars(table.cell_value(n, 12), self.global_variable)
                    ),
                ),
                13: (
                    "setup",
