...
import re
import os
from common.control.exc import CaseError, AssertError
            ...
            try:
                for item in expected_data_list:
                    ...
                    if i['type'] == "response_body":
                        self.log.info("进入断言 -> (response_body)验证response body中任意属性的值")
                        ...
                        self.log.info("进入断言 -> (response_incloud)验证response body中是否包含预期字符串")
                        assert self.assert_in_text(data, i['text'])
                        continue
                    elif i['type'] == "response_not_incloud":
                        self.log.info("进入断言 -> (response_not_incloud)验证response body中是不否包含预期字符串")
                        ...
                        self.log.info("进入断言 -> (time)验证response body响应时间小于预期最大响应时间,单位：毫秒")
                        assert self.assert_time(data["time_total"], i['text'])
                        continue
                    elif i['type'] == "mysql":
                        self.log.info("进入断言 -> (mysql)验证返回数据是否与数据库相一致")
                        assert self.assert_sql(i)
                        continue
                    elif i['type'] == "mongodb":
                        self.log.info("进入断言 -> (mongodb)验证返回数据是否与数据库相一致")
        ...
        if len(_expect) == 1:
            ...
            if isinstance(res,list):
                if 'len' in expected_data:
                    self.log.info("预期值: {0}, 实际返回值: {1}".format(expected_data['len'], len(res)))
                    ...
                    db = mysql.MysqlDb(database=expected_data["database"], project=self.project, env=self.env)
            ...
            raise AssertError('断言数据解析失败,格式错误,length_equals:text只支持一个参数')
...
if __name__ == '__main__':
    # data = Assertions(filename='data/starship/suite/smoketesting/suite_branch_commit.xlsx')
    # data.assert_sql({"type":"mysql","database":"api_test_starship_branch","sql":"select id from feature where id= '39ffd37b-6be7-db7d-072b-5bc325074bdb'","text":"result=[{'id': '39ffd37b-6be7-db7d-072b-5bc325074bdb'}]"})
    # data.assert_sql({"type":"mysql","database":"api_test_starship_branch","sql":"select id,name from feature where id= '39ffd37b-6be7-db7d-072b-5bc325074bdb'","text":"result[0].id=39ffd37b-6be7-db7d-072b-5bc325074bdb"})
    ...
    # data = {'type': 'length_equals', 'text': 'res=body.data', 'len': 1}
    # data = {"type":"length_equals","text":"res=body.data", "database":"tenant_skyline","sql":"select count(*) from product_product where id ='567040716023271424';"}
    # data = {"type":"length_equals","text":"res=body.data", "database":"tenant_skyline","sql":"select count(*) from product_product where id =${pid};"}
    # data = {"type":"length_equals","text":"res=body.data", "func":"${get_num_data()}"}
    data = {"type":"length_equals","text":"res=body.data", "func":"${get_num_data(num=${num})}"}
    test = Assertions('fast','conf',{'username': 'pengda01', 'password': '123456.0', 'url': 'https://mks-test.mypaas.com/auth-api/v2/user/login', 'now': '{"from": "2023-05-10 00:00:00", "to": "2023-05-10 16:34:01"}', 'token': '6af2a4d75d158dd37184b57f9278acb56d72fdd7', 'id': 'id', 'now_page': '{"from": "2023-05-10 00:00:00", "to": "2023-05-10 16:34:02", "page_index": 1, "page_size": "5"}', 'feature_id': [{'id': 574252882044391424}, {'id': 574260075980394496}], 'feature_id_2': [{'id': 574252882044391424}, {'id': 574260075980394496}], 'now_space': '{"from": "2023-05-10 00:00:00", "to": "2023-05-10 16:34:10", "space": 3600}',"pid":"567040716023271424","num":1})
    cc = test.length_equals(body,data)
    print(cc)
